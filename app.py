...
st.title("Обработка пропущенных значений")
st.markdown("---")

# Проверка наличия данных
if 'df' not in st.session_state or st.session_state.df.empty:
    st.warning("Данные не загружены. Пожалуйста, загрузите данные на предыдущем шаге.")
    st.stop()
...
if st.button("Обработать пропущенные значения", type="primary"):
    with st.spinner("Обработка данных..."):
        ...
        if st.session_state.df.isna().sum().sum() > 0:
            st.warning("В данных остались пропущенные значения! Рассмотрите другие методы обработки.")
        else:
            st.success("Все пропущенные значения успешно обработаны!")
#--
